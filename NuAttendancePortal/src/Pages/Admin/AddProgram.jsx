// src/Pages/Admin/AddProgram.jsx
...
const AddProgram = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await axios.post('http://localhost:4000/program/uploadProgramExcel', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });

      const uploadResults = res.data.programs?.map(p => ({
        p_id: p.p_id,
        status: 'success',
      ...
      if (uploadResults.length === 0) {
        ...
        setSuccess(`Programs inserted successfully! Total Inserted: ${uploadResults.length}`);
        setError('');
      }
      ...
      const results = err.response?.data?.errors?.map((e, index) => ({
        p_id: '',
        row: e.row,
        status: 'failed',
        error: e.error
    ...
    <div className="add-student-container">
      <div className="sidebar">
        ...
        <nav className="nav-list">
          ...
          <a href="/admin/add-program" className="nav-link">
            <span className="link-icon"><FaList /></span> Add Program
          </a>
        </nav>
      </div>
      ...
      <div className="main-content">
        <div className="upload-section">
          <h2 className="section-title">Upload Program Excel File</h2>
              ...
              <table className="result-table">
                ...
                <tbody>
                  {results.map((res, i) => (
                    <tr key={i}>
                      <td>{i + 1}</td>
                      <td>{res.p_id || `Row ${res.row}`}</td>
                      <td>{res.status}</td>
                      <td>{res.message || res.error || 'Inserted successfully'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
